  ...
  return (
    <Box sx={{ my: 4, mx: 4 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" sx={{ pb: 2 }}>
        <Typography variant="h6" color="primary" sx={{fontWeight:"bold",fontSize:"1.7rem"}}>Directory of Events</Typography>
      ...
      <ToggleButtonGroup
        ...
        sx={{ mb: 2, "& .MuiToggleButton-root": { px: 1.5, py: 0.5, fontSize: "0.875rem" } }}
      >
      ...
      {view === "table" ? (
        <TableContainer component={Paper}>
          <Table>
              ...
              <TableRow>
                {header.map((val, i) => (
                  <TableCell key={i} align={val === "Name" ? "left" : "right"}>
                    <TableSortLabel active={orderBy === val} direction={order} onClick={createSortHandler(val)} sx={{ fontWeight: "bold", fontSize: "1.05rem" }}>
                      {val}
                      {
                        orderBy === val && <Box component="span" sx={visuallyHidden}>{order === "desc" ? "sorted descending" : "sorted ascending"}</Box>
                      }
                    </TableSortLabel>
                  </TableCell>
                ))}
            ...
            <TableBody>
              {getSortedEvents(events).filter(event => event.name.toLowerCase().includes(search)).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((event) => (
                <TableRow key={event.id} sx={{ '&:hover': { backgroundColor: "rgba(0,0,0,0.1)" } }}>
                  <TableCell>{event.name}</TableCell>
                  <TableCell align="right">{event.date}</TableCell>
                  <TableCell align="right">{event.type}</TableCell>
                  <TableCell align="right" sx={{ color: event.status.includes("Past") ? red[500] : green[600] }}>{event.status}</TableCell>
                  <TableCell align="right"><Link to={`/events/${event.slug}`}>See more</Link></TableCell>
                </TableRow>
              ))}
            </TableBody>
            <TableFooter>
              <TableRow>
                <TablePagination rowsPerPageOptions={[10, 15, 20]} count={events.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} />
              </TableRow>
            </TableFooter>
          </Table>
        </TableContainer>
                ...
                <Card
                  sx={{
                    ...
                    '&:before': {
                      ...
                      width: '100%',
                      height: '4px',
                      background: 'linear-gradient(90deg,rgb(4, 71, 52) 0%, #36856F 100%)',
                    },
                  }}
                >
                  <CardActionArea 
                    ...
                    <CardContent sx={{ 
                      ...
                      <Box sx={{ mt: 'auto' }}>
                        <Box sx={{ 
                          ...
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <EventIcon sx={{ color: '#36856F', fontSize: 18, mr: 1 }} />
